...
class FeatExt(nn.Module):
    ...
    def __init__(self):
        ...
        feat_ext_dict = {k[16:]:v for k,v in torch.load('pretrained_model/vismvsnet.pt')['state_dict'].items() if k.startswith('module.feat_ext')}
...
def gen_camera_between(pose_0, pose_1, ratio):
    rot_0 = pose_0[:3, :3]
    rot_1 = pose_1[:3, :3]
...
def get_local_loss(diff_surf_pts,
                  uncerts,
                  feat,
                  cam,
                  feat_src,
    ...
    for view_i, slice_ in enumerate(slices):
        if slice_.start < slice_.stop:
            ...
            pts_world = (diff_surf_pts_slice / 2 * size.view(1, 1) + center.view(1, 3)).view(1, -1, 1, 3, 1)
            pts_world = torch.cat([pts_world, torch.ones_like(pts_world[..., -1:, :])], dim=-2)
            cam_pack = torch.cat([cam[view_i:view_i + 1], src_cams[view_i]], dim=0)
