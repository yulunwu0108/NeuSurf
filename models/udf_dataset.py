...
def process_data(data_dir, dataname):
    if os.path.exists(os.path.join(data_dir, 'pcd', dataname) + '.ply'):
        pointcloud = trimesh.load(os.path.join(data_dir, 'pcd', dataname) + '.ply').vertices
        pointcloud = np.asarray(pointcloud)
    elif os.path.exists(os.path.join(data_dir, 'pcd', dataname) + '.xyz'):
        pointcloud = np.loadtxt(os.path.join(data_dir, 'pcd', dataname) + '.xyz')
    elif os.path.exists(os.path.join(data_dir, 'pcd', dataname) + '.npy'):
        pointcloud = np.load(os.path.join(data_dir, 'pcd', dataname) + '.npy')
    else:
...
class Dataset:
    def __init__(self, conf, dataname):
        ...
        self.data_dir = conf.get_string('data_dir').replace('CASE_NAME', dataname)
        print(self.data_dir)
