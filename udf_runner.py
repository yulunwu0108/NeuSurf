...
from models.udf_dataset import Dataset
...
class UDFRunner:
    def __init__(self, args, conf_path):
        ...
        self.base_exp_dir = (self.conf['general.base_dir'] + args.udf_dir).replace('CASE_NAME', args.case)
        os.makedirs(self.base_exp_dir, exist_ok=True)


        self.dataset = Dataset(self.conf['udf_dataset'], args.case)
        self.dataname = args.case
        self.iter_step = 0

        # Training parameters
        self.step1_maxiter = self.conf.get_int('udf_train.step1_maxiter')
        self.step2_maxiter = self.conf.get_int('udf_train.step2_maxiter')
        self.save_freq = self.conf.get_int('udf_train.save_freq')
        self.report_freq = self.conf.get_int('udf_train.report_freq')
        self.val_freq = self.conf.get_int('udf_train.val_freq')
        self.val_mesh_freq = self.conf.get_int('udf_train.val_mesh_freq')
        self.batch_size = self.conf.get_int('udf_train.batch_size')
        self.batch_size_step2 = self.conf.get_int('udf_train.batch_size_step2')
        self.learning_rate = self.conf.get_float('udf_train.learning_rate')
        self.warm_up_end = self.conf.get_float('udf_train.warm_up_end', default=0.0)
        self.eval_num_points = self.conf.get_int('udf_train.eval_num_points')
        self.df_filter = self.conf.get_float('udf_train.df_filter')

        self.ChamferDisL1 = ChamferDistanceL1().cuda()
        self.ChamferDisL2 = ChamferDistanceL2().cuda()

        # Weights
        self.igr_weight = self.conf.get_float('udf_train.igr_weight')
        self.mask_weight = self.conf.get_float('udf_train.mask_weight')
        self.model_list = []
        self.writer = None

        # Networks
        self.udf_network = UDFNetwork(**self.conf['udf_model.udf_network']).to(self.device)
        if self.conf.get_string('udf_train.load_ckpt') != 'none':
            self.udf_network.load_state_dict(torch.load(self.conf.get_string('udf_train.load_ckpt'), map_location=self.device)["udf_network_fine"])
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(self.iter_step, self.step2_maxiter)):
            ...
            if self.iter_step == self.step1_maxiter:
                gen_pointclouds = self.gen_extra_pointcloud(self.iter_step, self.conf.get_float('udf_train.low_range'))
                idx = pcu.downsample_point_cloud_poisson_disk(gen_pointclouds, num_samples=int(self.conf.get_float('udf_train.extra_points_rate')*point_gt.shape[0]))
    ...
    def extract_mesh(self, resolution=64, threshold=0.0, point_gt=None, iter_step=0, logger=None):
        ...
        if self.conf.get_float('udf_train.far') > 0:
            mesh = remove_far(point_gt.detach().cpu().numpy(), mesh, self.conf.get_float('udf_train.far'))
    ...
    def gen_extra_pointcloud(self, iter_step, low_range):
        ...
        res = remove_outlier(point_gt.detach().cpu().numpy(), res, dis_trunc=self.conf.get_float('udf_train.outlier'))
        return res
...
if __name__ == '__main__':
    ...
    runner = UDFRunner(args, args.conf)
